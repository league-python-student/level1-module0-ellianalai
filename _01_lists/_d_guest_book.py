...
def add_guest(list_of_guests):
    person = simpledialog.askstring(title = 'Guestbook', prompt = 'What is the name of the guest you would like to add?' )
    list_of_guests.append(person)
...
def print_guests(list_of_guests):
    ...
    if len(list_of_guests)== 0:
        messagebox.showinfo(title = 'Guest List', message = 'There are no guests')
    else:
        messagebox.showinfo(title='Guest List', message = str)
